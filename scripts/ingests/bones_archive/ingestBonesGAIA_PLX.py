...
for source in bones_sheet_table:
    ...
    if len(match) == 1:
        ...
        try:
            band_filter = "GAIA"
            measurement = source["GAIA_PLX"] 
            telescope = "Gaia"
            reference = "GaiaDR2"
            error = source["GAIA_PLX_E"]
...
logger.info(f"skipped:{skipped}") # 144 skipped
logger.info(f"photometry_ingested:{astrometry_ingested}") # 65 ingsted
logger.info(f"total: {total}") # 209 total
logger.info(f"no_sources:{no_sources}") # 49 no sources
